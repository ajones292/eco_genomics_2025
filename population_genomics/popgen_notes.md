...
### 09/23/25: Calculate average diversity statisitics collected from ANGSD and estimate effective population size

Created R Markdown file to make calculations and visualize genomic diversity:

`~/projects/eco_genomics_2025/population_genomics/mydocs/Nucleotide_Diversity.Rmd`

Read in the output file generated by the final `cut` command in `ANGSD_doTheta.sh`:

`~/projects/eco_genomics_2025/population_genomics/myresults/ANGSD/diversity/2021_ALL_win50000_step50000.thetas`

Scaled Watterson's Theta (tW) and Theta Pi (tP) by the number of sites identified (nSites). Generated rows tWsite and tPsite.

Plotted a histogram to visualize the number of polymorphic sites in each sequenced window. Median number of sites in each window is 894.1.

Plotted nucleotide diversity across scaffolds as Theta Pi (tPsite). Fairly even distribution of nucleotide diversity across scaffolds.

Plotted scaled values for Theta Pi (tPsite) and Watterson's Theta (tWsite) together against the theoretical 1-to-1 relationship achieved in the neutral model of population equilibrium. Theta Pi is consistently greater than Watterson's Theta, indicating balancing selection in this population.
