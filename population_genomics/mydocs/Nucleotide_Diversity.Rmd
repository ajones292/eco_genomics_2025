...
knitr::opts_knit$set(root.dir = "~/projects/eco_genomics_2025/population_genomics/myresults/ANGSD_RunD/diversity/")
```
...
```{r}
theta <- read.table("2021_ALL_win50000_step50000.thetas", sep="\t", header=T)

head(theta)
```

First, we need to scale our theata by nSites
...
ggplot(theta2, aes(x=Chr, y=tPsite, color=Chr)) +
  ...
  ylim(0,0.07) +
  labs(title="Nucleotide diversity in 50 kb windows for population 2021", x="Scaffold", y="Theta-Pi")
...
ggplot(theta2, aes(x=tWsite, y=tPsite, color=Chr)) +
  ...
  labs(title="Pairwise differences vs. segregating sites for population 2021", x="Theta-W", y="Theta-Pi")
